...
pub trait TapExt<'a>: Observable<'a> + Sized {
    fn tap<M>(self, tap: M) -> TapObservable<M, Self> where M: Fn(&Self::Item) + Send + Sync + 'a {
...
impl<'a, T, O> Observable<'a> for TapObservable<T, O> where O: Observable<'a> + 'a, T: Fn(&O::Item) + Send + Sync + 'a {
    type Item = O::Item;
    type Error = O::Error;
